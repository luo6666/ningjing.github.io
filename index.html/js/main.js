(function() {
	...
	function PopStar() {
		...
		// prototype属性向对象添加属性和方法 object.prototype.name=value  init()是自定义方法
	}
	...
	PopStar.prototype = {
		...
		initScore: function() {
			...
			// config.scoreTarget 目标分数   start()？？？
			config.scoreCurrent.innerHTML = computed.totalScore;
			// config.scoreCurrent 当前分数div盒子  computed.totalScore 总分数
		 ...
		 */
		mouseClick: function() {
			var tileSet = config.tileSet,//存储小星星的二维数组
				choose = computed.choose,//已选中的小星星集合
				baseScore = config.baseScore,//每一个小星星的基础分数
				stepScore = config.stepScore,//每一个小星星的递增分数
				el = config.el,// 星星列表
				self = this,
				len = choose.length;//已选中的小星星集合的长度
			if (!computed.flag || len <= 1) {//computed.flag：true 锁   （！取反符 computed.flag：flase） len 已选中的小星星集合的长度<=1
				return;
			}
			computed.flag = false;//不锁
				...
				setTimeout(function() {
					if (self.isFinish()) {
						...
						if (computed.totalScore >= config.targetScore) {
							new CountUp(config.scoreTarget, config.targetScore, config.targetScore += (computed.level - 1) * computed .stepTargetScore) .start();
							new CountUp(config.scoreLevel, computed.level, computed.level += 1).start();
							alert("恭喜获胜");
							console.log("恭喜获胜")
						} else {
							config.targetScore = config.scoreTarget = 1500;
		...
		clear: function() {
			var tileSet = config.tileSet,//存储小星星的二维数组
				rows = tileSet.length,//存储小星星的二维数组的长度
				el = config.el; // 星星列表
			var temp = [];
			for (var i = rows - 1; i >= 0; i--) {
				for (var j = tileSet[i].length - 1; j >= 0; j--) {
					...
					temp.push(tileSet[i][j])//在temp数组中添加已被选中的小星星列
					tileSet[i][j] = null;//将被选中的没有小星星消掉
				}
			}
